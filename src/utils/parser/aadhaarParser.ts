export interface AadhaarData {
  ...
  export const parseAadhaarText = (frontText: string, backText: string): AadhaarData => {
    ...
    if (possibleName) {
      ...
      const altName = frontLines.find(line =>
        /^[a-z]{2,}(?: [a-z]{2,}){1,2}$/.test(line) && !/india|government/i.test(line)
      );
      if (altName) {
        data.name = altName
          .split(' ')
